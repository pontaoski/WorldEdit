...
namespace WorldEdit
{
    ...
    public class WorldEdit : TerrariaPlugin
    {
            ...
            if (!e.Handled && e.MsgID == PacketTypes.Tile)
            {
                ...
                if (info.pt != 0)
                {
                    ...
                    if (X >= 0 && Y >= 0 && X < Main.maxTilesX && Y < Main.maxTilesY)
                    {
                        ...
                        {
                            List<string> Reg = regionManager.InAreaRegionName(X, Y);
                            if (Reg.Count == 0)
                            ...
                            curReg = regionManager.GetRegionByName(Reg[0]);
                                ...
                                TShock.Players[e.Msg.whoAmI].SendMessage(String.Format("Region Set.", X, Y), Color.Yellow);
                        }
                        else
                        {
                            info.x2 = X;
                            info.y2 = Y;
                            TShock.Players[e.Msg.whoAmI].SendMessage("New", Color.Yellow);
                        }
                        info.pt = 0;
                        e.Handled = true;
        ...
        void Flood(CommandArgs e)
        {
            ...
            int y = e.Player.TileY - radius + 1;
            int y2 = e.Player.TileY + radius + 1;
            CommandQueue.Add(new FloodCommand(x, y, x2, y2, e.Player.Index, liquid == "lava"));
        }
        ...
        void Region(CommandArgs e)
        {
            ...
            if (e.Parameters.Count > 2)
            {
                e.Player.SendMessage("Invalid syntax: //region //region <Region Name> or //region <X> <Y>", Color.Red);
                return;
            }
            else if (e.Parameters.Count == 0)
            {
                Players[e.Player.Index].pt = 3;
                e.Player.SendMessage("Hit a block to use that region.", Color.Yellow);
            ...
            else if (e.Parameters.Count == 1)
            {
                ...
                PlayerInfo info = Players[e.Player.Index];
                curReg = regionManager.ZacksGetRegionByName(e.Parameters[0].ToLower());
                if (curReg != null)
                {
                    e.Player.SendMessage("Invalid Region!");
                    return;
                }
                else
                {
            ...
            else if (e.Parameters.Count == 2)
            {
                ...
                PlayerInfo info = Players[e.Player.Index];
                List<string> regions = regionManager.InAreaRegionName(Int32.Parse(e.Parameters[0]), Int32.Parse(e.Parameters[1]));
                if (regions.Count >= 1)
                {
                    if (regions.Count > 1)
                    {
                ...
                {
                    e.Player.SendMessage("Invalid Region: No Region Found");
                    return;
                }
            }
            else
            {
                e.Player.SendMessage("Invalid Parameters: " + e.Parameters.Count);
            }
        }
        ...
        void Replace(CommandArgs e)
        {
            ...
            {
                e.Player.SendMessage("More than one tile matched.", Color.Red);
            }
            ...
            {
                CommandQueue.Add(new ReplaceCommand(info.x, info.y, info.x2, info.y2, e.Player.Index, values1[0], values2[0]));
            }
        }
        void ReplaceWall(CommandArgs e)
        {
            ...
            {
                e.Player.SendMessage("More than one wall matched.", Color.Red);
            }
            else if (values2.Count == 0)
            {
                e.Player.SendMessage("Invalid wall.", Color.Red);
            ...
            {
                CommandQueue.Add(new ReplaceWallCommand(info.x, info.y, info.x2, info.y2, e.Player.Index, values1[0], values2[0]));
            }
        }
        ...
        void Schematic(CommandArgs e)
        {
            ...
            switch (subCmd)
            {
                ...
                case "delete":
                    ...
                    break;
                case "help":
                    e.Player.SendMessage("Schematics commands:", Color.Green);
        ...
        void Set(CommandArgs e)
        {
            if (e.Parameters.Count != 1)
            {
                e.Player.SendMessage("Invalid syntax! Proper syntax: //set <tile>", Color.Red);
                return;
            }
            PlayerInfo info = Players[e.Player.Index];
            if (info.x == -1 || info.y == -1 || info.x2 == -1 || info.y2 == -1)
            ...
            {
                CommandQueue.Add(new SetCommand(info.x, info.y, info.x2, info.y2, e.Player.Index, values[0]));
            }
        }
        void SetWall(CommandArgs e)
        {
            if (e.Parameters.Count != 1)
            {
                e.Player.SendMessage("Invalid syntax! Proper syntax: //setwall <wall>", Color.Red);
                return;
            }
            PlayerInfo info = Players[e.Player.Index];
            if (info.x == -1 || info.y == -1 || info.x2 == -1 || info.y2 == -1)
            ...
            {
                CommandQueue.Add(new SetWallCommand(info.x, info.y, info.x2, info.y2, e.Player.Index, values[0]));
            }
        }
