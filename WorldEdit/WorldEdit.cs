...
namespace WorldEdit
{
	...
	public class WorldEdit : TerrariaPlugin
	{
				...
				if (info.pt != 0)
				{
					...
					if (X >= 0 && Y >= 0 && X < Main.maxTilesX && Y < Main.maxTilesY)
					{
						if (info.pt == 1)
						{
							info.x = X;
							info.y = Y;
							TShock.Players[e.Msg.whoAmI].SendMessage("Set point 1.", Color.Yellow);
						}
						else if (info.pt == 3)
						{
							...
							if (Reg.Count == 0)
							{
								TShock.Players[e.Msg.whoAmI].SendMessage("No region exists there.", Color.Red);
								return;
							}
							...
							TShock.Players[e.Msg.whoAmI].SendMessage("Set region.", Color.Yellow);
						}
						else
						{
							info.x2 = X;
							info.y2 = Y;
							TShock.Players[e.Msg.whoAmI].SendMessage("Set point 2.", Color.Yellow);
						}
						info.pt = 0;
						e.Handled = true;
		...
		void Region(CommandArgs e)
		{
			if (e.Parameters.Count > 2)
			{
				e.Player.SendMessage("Invalid syntax! Proper syntax: //region [region name] | [x] [y]", Color.Red);
				return;
			}
			...
			else
			{
				...
				{
					e.Player.SendMessage("Invalid coordinates.", Color.Red);
					return;
				}
				...
				if (regions.Count >= 1)
				{
					if (regions.Count > 1)
					{
						e.Player.SendMessage("Overlapping regions; using first encountered...", Color.Yellow);
						/*int q = 0;
